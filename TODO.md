...
### 6. SDK `google.generativeai` đã deprecated
...
### 7. `.claude/settings.local.json` đang trong git
...
### 8. AI services dùng `print()` thay vì logging
...
### 9. Xóa `set_unusable_password()` thừa
...
### 10. OTP flow: lấy username từ email bằng `split('@')`
